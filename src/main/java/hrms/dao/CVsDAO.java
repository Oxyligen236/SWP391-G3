...
public class CVsDAO extends DBContext {
    ...
    public void updateCV(CVs cv) {
        String sql = "update CVs set jdID = ?, name = ?, email = ?, phone = ?, cv_Description = ?, status = ? where cv_ID = ?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
    ...
    public boolean addCV(CVs cv) {
        String sql = "insert into CVs (jobID, name, mail, phone_number, cv_Description, status) values (?, ?, ?, ?, ?, ?)";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, cv.getJdID());
            st.setString(2, cv.getName());
    ...
    public List<CVJobDetailDTO> getAllCVJobTitle() {
        String sql = "select cv.cv_ID,jd.jobid, jd.jobTitle, cv.name, cv.mail, cv.phone_number, cv.cv_Description, cv.status "
    ...
    public List<CVJobDetailDTO> searchCVs(String name, String email, String phone) {
        StringBuilder sql = new StringBuilder("select cv.cv_ID,jd.jobid, jd.jobTitle, cv.name, cv.mail, cv.phone_number, cv.cv_Description, cv.status "
