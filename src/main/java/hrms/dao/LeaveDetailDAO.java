...
public class LeaveDetailDAO extends DBContext {
    ...
    public boolean add(LeaveDetail detail) {
        String sql = "INSERT INTO LeaveDetail (TicketID, Start_Date, End_Date, LeaveType) " +
                     "VALUES (?, ?, ?, ?)";
        try {
            ...
            st.setString(4, detail.getLeaveType());
    ...
    public LeaveDetail getByTicketId(int ticketId) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                detail.setLeaveType(rs.getString("LeaveType"));
                return detail;
            }
        } catch (SQLException e) {
    ...
    public List<LeaveDetail> getByLeaveType(String leaveType) {
        List<LeaveDetail> list = new ArrayList<>();
        String sql = "SELECT * FROM LeaveDetail WHERE LeaveType = ? ORDER BY Start_Date DESC";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, leaveType);
            ResultSet rs = st.executeQuery();
            ...
            System.err.println("Error in LeaveDetailDAO.getByLeaveType: " + e.getMessage());
            e.printStackTrace();
        }
        return list;
    }
