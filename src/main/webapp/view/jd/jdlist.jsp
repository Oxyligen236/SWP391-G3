            ...
            <form class="row g-3 mb-3 align-items-end" method="get" action="<c:url value='/jdlist'/>">
                ...
                <div class="col-md-3">
                    ...
                    <select name="department" id="department" class="form-select" onchange="this.form.submit()">
                        <option value="" ${param.department == '' ? 'selected' : ''}>All</option>
                        <c:forEach var="d" items="${departments}">
                            <option value="${d.name}" ${param.department == d.name ? 'selected' : ''}>${d.name}</option>
                        </c:forEach>
                    </select>
...
<td><a href="jd_detail?id=${jd.jobID}" class="btn btn-sm btn-primary">View</a><c:choose><c:when test="${jd.status == 'InProgress'}"><a href="pendingjd?id=${jd.jobID}" class="btn btn-sm btn-warning" onclick="return confirm('Suspend this JD?');">Suspend</a></c:when><c:when test="${jd.status == 'Pending'}"><a href=reopenjd?id=${jd.jobID} class="btn btn-sm btn-success" onclick="return confirm('Reopen this JD?');">Open</a></c:when></c:choose><c:if test="${jd.status != 'Cancelled'}"><a href="canceljd?id=${jd.jobID}" class="btn btn-sm btn-danger" onclick="return confirm('Cancel this JD?');">Cancel</a></c:if></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
